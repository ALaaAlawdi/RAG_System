...
class OCRClient:
    ...
    def _build_payload(self, output_format: str, langs: str) -> dict:
        ...
        return {
            ...
            "langs":                  langs,
            "skip_cache":             b(settings.OCR_SKIP_CACHE),
            "table_row_bboxes":       b(settings.OCR_TABLE_ROW_BBOXES),
            "disable_image_captions": b(settings.OCR_DISABLE_IMAGE_CAPTIONS),
            "extract_links":          b(settings.OCR_EXTRACT_LINKS),
            "paginate":               b(settings.OCR_PAGINATE),
            "keep_page_header":       b(settings.OCR_KEEP_PAGE_HEADER),
            "keep_page_footer":       b(settings.OCR_KEEP_PAGE_FOOTER),
            "new_block_types":        b(settings.OCR_NEW_BLOCK_TYPES),
        }
    ...
    async def process_document(
        ...
        resolved_format = output_format or settings.OCR_OUTPUT_FORMAT
        resolved_langs  = langs         or settings.OCR_LANGS
    ...
    async def _poll(self, check_url: str) -> OCRStatusResponse:
        max_attempts = settings.OCR_MAX_POLL_ATTEMPTS
        interval     = settings.OCR_POLL_INTERVAL_SECONDS

        async with httpx.AsyncClient() as client:
            for attempt in range(1, max_attempts + 1):
                logger.debug(f"[OCR] Poll {attempt}/{max_attempts}")
                ...
                logger.debug(f"[OCR] Status='{data.status}', sleeping {interval}s")
                await asyncio.sleep(interval)

        raise OCRTimeoutError(
            f"OCR polling timed out after {max_attempts} attempts "
